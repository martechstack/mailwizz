...
/**
 * ListFieldBuilderTypeTextSubscriber
 *
...
/**
 * @property ListFieldBuilderTypeText $owner
 */
class ListFieldBuilderTypeTextSubscriber extends ListFieldBuilderTypeSubscriber
{
    /**
     * @param CEvent $event
     *
    ...
    public function _setCorrectValidationRules(CModelEvent $event)
    {
        ...
        if ($event->sender->field->tag === 'EMAIL') {
            ...
            $validator = 'customer.components.list-field-builder.text.validators.ListFieldBuilderTypeTextEmailValidator';
            $rules->add(['value', 'email', 'checkMX' => $checkMX, 'checkPort' => $checkPort]);
            $rules->add(['value', $validator, 'field' => $event->sender->field, 'subscriber' => $this->owner->getSubscriber()]);
        }
    }
    ...
    protected function buildFieldArray(ListFieldValue $model): array
    {
        ...
        return hooks()->applyFilters('list_field_builder_type_text_subscriber_build_field_array', [
