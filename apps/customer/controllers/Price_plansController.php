...
class Price_plansController extends Controller
{
    ...
    public function actionOrder_pdf($order_uid)
    {
        ...
        // 1.8.4
        $html = hooks()->applyFilters('price_plan_order_generate_pdf_invoice', $html, $order);
    ...
    public function actionPayment()
    {
        ...
        if ($in->itemat('overLimit') === true) {
            $reason = t('price_plans', 'Selected price plan allows {n} {w} but you already have {m}, therefore you cannot apply for the plan!', [
                '{n}' => $in->itemAt('limit'),
                '{w}' => t('price_plans', $in->itemAt('object')),
                '{m}' => $in->itemAt('count'),
            ]);
        ...
        /** @var PricePlanPromoCode $promoCodeModel */
        $promoCodeModel = null;
        if (!empty($promoCode)) {
    ...
    public function actionOrder()
    {
        ...
        $order->onAfterSave = [$this, '_sendOrderNotifications'];
     ...
     * @throws CException
     */
    public function _sendOrderNotifications(CEvent $event)
    {
        /** @var PricePlanOrder $order */
        $order = $event->sender;
