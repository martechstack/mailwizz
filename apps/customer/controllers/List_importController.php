...
class List_importController extends Controller
{
    ...
    public function init()
    {
        ...
        if (!container()->get(OptionImporter::class)->getIsEnabled()) {
    ...
    public function actionCsv($list_uid)
    {
        ...
        try {
            ...
            $csvHeader = ioFilter()->stripPurify($csvHeader);
            ...
            foreach ($csvData as $row) {
                ...
                foreach ($row as $name => $value) {
                    ...
                    $rowData[] = [
                        'name'      => ucwords((string)str_replace('_', ' ', $name)),
            ...
            $data        = $collection->itemAt('data');
            $importLog   = $collection->itemAt('importLog');
            $importCount = $collection->itemAt('importCount');
            $finished    = $collection->itemAt('finished');
            $failures    = $collection->itemAt('failures');
            //
    ...
    protected function handleQueuePlacement(string $list_uid, string $importClass = 'ListCsvImport')
    {
        ...
        $import->file_size_limit = container()->get(OptionImporter::class)->getFileSizeLimit();
        $import->attributes      = (array)request()->getPost($import->getModelName(), []);
        $import->file            = CUploadedFile::getInstance($import, 'file');
