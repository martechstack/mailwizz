...
class CampaignsController extends Controller
{
    ...
    public function actionSetup($campaign_uid)
    {
        ...
        if (request()->getIsPostRequest() && ($attributes = (array)request()->getPost($campaign->getModelName(), []))) {
            ...
            $unfiltered = request()->getOriginalPost($campaign->getModelName(), []);
            if (isset($unfiltered['subject'])) {
                $campaign->subject = html_decode(strip_tags(ioFilter()->purify(html_decode($unfiltered['subject']))));
            }
            if ($campaign->save() && $campaign->option->save()) {
                ...
                if ($postAttributes = request()->getPost($openUnopenFilter->getModelName(), [])) {
    ...
    public function actionTemplate($campaign_uid, $do = 'create')
    {
        ...
        if ($do === 'select') {
            ...
            if (request()->getIsPostRequest() && ($attributes = (array)request()->getPost($template->getModelName(), []))) {
                ...
                if (isset(app_param('POST')[$template->getModelName()]['content'])) {
                    $template->content = app_param('POST')[$template->getModelName()]['content'];
                } else {
                    $template->content = '';
                }
                ...
                if ($do === 'from-url' && isset($attributes['from_url'])) {
                    if (!FilterVarHelper::url($attributes['from_url'])) {
                        ...
                        $response      = '';
                        try {
                            $response = (string)(new GuzzleHttp\Client())->get($attributes['from_url'])->getBody();
                        } catch (Exception $e) {
                ...
                if ($template->getIsOnlyPlainText()) {
                    $template->content    = html_decode(ioFilter()->purify($template->plain_text));
                ...
                if (empty($errors) && $template->save()) {
                    ...
                    if ($postAttributes = (array)request()->getPost('CampaignRandomContent', [])) {
                        foreach ($postAttributes as $index => $attributes) {
                            ...
                            $rndContent->content     = app_param('POST')['CampaignRandomContent'][$index]['content'];
                            try {
                                $rndContent->save();
                            } catch (Exception $e) {
                            }
    ...
    public function actionTest($campaign_uid)
    {
        ...
        foreach ($emails as $email) {
            ...
            if (empty($emailSubject)) {
                $emailSubject   = '[' . strtoupper(t('app', 'Test')) . '] ' . $campaign->name;
            }
            ...
            for ($i = 0; $i < 3; ++$i) {
                ...
                /** @var DeliveryServer $server */
                $server = DeliveryServer::pickServer((int)$server->server_id, $campaign, $dsParams);
    ...
    public function actionSync_datetime()
    {
        ...
        $timeZoneDateTime   = date('Y-m-d H:i:s', (int)strtotime(request()->getQuery('date', date('Y-m-d H:i:s'))));
    ...
    public function actionShare_reports_send_email($campaign_uid)
    {
        ...
        $params = CommonEmailTemplate::getAsParamsArrayBySlug(
            ...
            [
                'to'      => [$shareReports->share_reports_email => $shareReports->share_reports_email],
        ...
        /** @var DeliveryServer $server */
        $server->setDeliveryFor(DeliveryServer::DELIVERY_FOR_CAMPAIGN)->setDeliveryObject($campaign)->sendEmail($params);

        if (empty($server)) {
            $this->renderJson([
                'result'  => 'error',
                'message' =>  t('campaigns', 'Unable to send the email at this time, please try again later!'),
            ]);
        }

        $this->renderJson([
            'result'  => 'success',
            'message' =>  t('campaigns', 'The email has been sent successfully!'),
        ]);
    }
    ...
    public function actionQuick_view($campaign_uid)
    {
        ...
        $this->renderPartial('_quick-view', compact('campaign'));
    }
