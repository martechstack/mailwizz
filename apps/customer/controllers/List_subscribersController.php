...
class List_subscribersController extends Controller
{
    ...
    public function actionIndex($list_uid)
    {
        ...
        $storedToggleColumns      = options()->get($optionKey, []);
        ...
        $columns[] = [
            ...
            'field_type'=> 'text',
            'field_id'  => 'uid',
            'value'     => isset($postFilter['uid']) ? html_encode($postFilter['uid']) : null,
        ];
        ...
        $columns[] = [
            ...
            'field_type'=> 'select',
            'field_id'  => 'status',
            'value'     => isset($postFilter['status']) ? html_encode($postFilter['status']) : null,
        ...
        foreach ($fields as $field) {
            $columns[] = [
                ...
                'value'     => isset($postFilter[$field->field_id]) ? html_encode($postFilter[$field->field_id]) : null,
            ];
        }
        ...
        foreach ($subscribers as $subscriber) {
            ...
            foreach ($fields as $field) {
                ...
                $subscriberRow['columns'][] = html_encode(implode(', ', $value));
            }
    ...
    public function actionBulk_action($list_uid)
    {
        ...
        $selectedSubscribers = $collection->itemAt('data');
        //
