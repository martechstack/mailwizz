...
class GuestController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if (GuestFailAttempt::model()->setBaseInfo()->getHasTooManyFailures()) {
            throw new CHttpException(403, t('app', 'Your access to this resource is forbidden.'));
        }

        // since 1.5.1
        $loginBgImage = container()->get(OptionCustomization::class)->getCustomerLoginBgUrl(5000, 5000);
            ...
            if ($model->validate()) {
                ...
                if (!$twoFaSettings->getIsEnabled() || !$customer->getTwoFaEnabled()) {
                    ...
                    if ($model->twofa_code) {
                        ...
                        /** @var mixed $previousTs */
                        $previousTs = $customer->twofa_timestamp;

                        $timestamp = $manager
                            ->setCycles(5)
                            ->verify($model->twofa_code, $customer->twofa_secret, $previousTs);
    ...
    public function actionConfirm_registration($key)
    {
        ...
        /** @var OptionEmailTemplate */
        $emailTemplateOption = container()->get(OptionEmailTemplate::class);
        ...
        if (!empty($sendWelcome) && !empty($sendWelcomeSubject) && !empty($sendWelcomeContent)) {
            ...
            $emailTemplate      = (string)str_replace('[CONTENT]', $sendWelcomeContent, $emailTemplateOption->common);
    ...
    public function actionFacebook()
    {
        ...
        $attributes = ioFilter()->stripClean($attributes);
    ...
    public function actionTwitter()
    {
        ...
        if (!isset($session['access_token'])) {
            ...
            $twitterOauth = new Abraham\TwitterOAuth\TwitterOAuth($appConsumerKey, $appConsumerSecret, $session['oauth_token'], $session['oauth_token_secret']);
        ...
        $twitterOauth = new Abraham\TwitterOAuth\TwitterOAuth($appConsumerKey, $appConsumerSecret, $accessToken['oauth_token'], $accessToken['oauth_token_secret']);
        ...
        if (($attributes = request()->getPost($customer->getModelName(), []))) {
