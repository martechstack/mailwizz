        ...
        <div class="box-header">
            ...
            <div class="pull-right">
                <?php BoxHeaderContent::make(BoxHeaderContent::RIGHT)
                    ...
                    ->add(CHtml::link(t('campaign_reports', 'All clicks'), [$controller->campaignReportsController . '/click', 'campaign_uid' => $campaign->campaign_uid], ['class' => 'btn btn-primary btn-flat', 'title' => t('campaign_reports', 'Back to clicks report')]))
                    ->add(CHtml::link(t('campaign_reports', 'Only unique urls'), [$controller->campaignReportsController . '/click_by_subscriber_unique', 'campaign_uid' => $campaign->campaign_uid, 'subscriber_uid' => $subscriber->subscriber_uid], ['class' => 'btn btn-primary btn-flat', 'title' => t('campaign_reports', 'Only unique urls')]))
                    ->addIf(CHtml::link(IconHelper::make('export') . t('campaign_reports', 'Export reports'), [$controller->campaignReportsExportController . '/click_by_subscriber', 'campaign_uid' => $campaign->campaign_uid, 'subscriber_uid' => $subscriber->subscriber_uid], ['target' => '_blank', 'class' => 'btn btn-primary btn-flat', 'title' => t('campaign_reports', 'Export reports')]), !empty($canExportStats))
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?php
                    $text = 'This page shows all the urls the subscriber <b>{email}</b> clicked on.<br />
                    If the subscriber clicked same link twice, you will see it twice so that you can compare the time when the click has been made.<br />
                    If you need to see only the unique urls and their clicks count for this subscriber, please click 
                    <a href="{href}">here</a>.';
                    echo t('campaign_reports', StringHelper::normalizeTranslationString($text), [
                        '{email}'   => $subscriber->getDisplayEmail(),
                        '{href}'    => createUrl('campaign_reports/click_by_subscriber_unique', ['campaign_uid' => $campaign->campaign_uid, 'subscriber_uid' => $subscriber->subscriber_uid]),
                    ]);
                    ?>
                </div>
            </div>
