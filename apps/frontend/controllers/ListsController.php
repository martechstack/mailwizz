...
class ListsController extends Controller
{
    ...
    public function actionSubscribe($list_uid, $subscriber_uid = null)
    {
        ...
        if (request()->getIsPostRequest() && strlen(request()->getPost($honeypotFieldName, '')) > 0) {
        ...
        $subscriber->ip_address = request()->getUserHostAddress();
        ...
        if (request()->getIsPostRequest()) {
            ...
            try {
                ...
                if ($duplicate) {
                    ...
                    if (app_param('validationSubscriberAlreadyExistsSubscriber')) {
        ...
        // since 1.3.5.6
        $content = hooks()->applyFilters('frontend_list_subscribe_before_transform_list_fields', $content);

        // list fields transform and handling
        $content = preg_replace('/\[LIST_FIELDS\]/', $fieldsHtml, $content, 1, $count);

        // since 1.3.5.6
        $content = hooks()->applyFilters('frontend_list_subscribe_after_transform_list_fields', $content);
    ...
    public function actionUpdate_profile($list_uid, $subscriber_uid)
    {
        ...
        $searchReplace = [
            ...
            '[UNSUBSCRIBE_URL]'     => container()->get(OptionUrl::class)->getFrontendUrl('lists/' . $list->list_uid . '/unsubscribe/' . $subscriber->subscriber_uid),
        ];
        ...
        if ($saved && !empty($campaign_uid)) {
            ...
            if (!empty($campaign)) {
                ...
                if (empty($track)) {
                    ...
                    $track->ip_address    = request()->getUserHostAddress();
                    $track->user_agent    = substr(request()->getUserAgent(), 0, 255);
                    // since 1.3.6.2
    ...
    public function actionBlock_address()
    {
        ...
        if (request()->getIsPostRequest() && ($attributes = (array)request()->getPost($model->getModelName(), []))) {
            ...
            if ($collection->itemAt('success')) {
                ...
                try {
                    ...
                    $detailsLink = CHtml::link(t('messages', 'here'), container()->get(OptionUrl::class)->getBackendUrl('block-email-request/index'));
        ...
        $appName = container()->get(OptionCommon::class)->getSiteName();
    ...
    public function setCustomerLanguage(Customer $customer): self
    {
        ...
        if (($langCode = request()->getQuery('lang')) && strlen($langCode) <= 5) {
            return $this;
        }

        // multilanguage is available since 1.1 and the Language class does not exist prior to that version
        if (!version_compare(container()->get(OptionCommon::class)->version, '1.1', '>=')) {
            return $this;
        }
