...
class CampaignsController extends Controller
{
    ...
    public function actionWeb_version($campaign_uid, $subscriber_uid = null)
    {
        ...
        $emailContent   = !empty($template) && !empty($template->content) ? $template->content : '';
        $emailHeader    = null;
        $emailFooter    = null;
    ...
    public function actionTrack_opening($campaign_uid, $subscriber_uid)
    {
        ...
        if (($ipAddress = request()->getUserHostAddress()) && FilterVarHelper::ip($ipAddress)) {
    ...
    public function actionForward_friend($campaign_uid, $subscriber_uid = null)
    {
        ...
        $forwardUrl  = container()->get(OptionUrl::class)->getFrontendUrl('campaigns/' . $campaign->campaign_uid);

        if (!empty($subscriber)) {
            $forward->from_email = $subscriber->email;
        }
        $forward->subject = t('campaigns', 'Hey, check out this url, i think you will like it.');

        if (request()->getIsPostRequest() && ($attributes = request()->getPost($forward->getModelName(), []))) {
    ...
    public function _actionCanTrackOpening($canTrack)
    {
        ...
        return $model->canTrackIpAction(request()->getUserHostAddress());
    }
    ...
    public function _actionCanTrackUrl($canTrack)
    {
        ...
        /** @var OptionCampaignExcludeIpsFromTracking $model */
        $model = container()->get(OptionCampaignExcludeIpsFromTracking::class);
        return $model->canTrackIpAction(request()->getUserHostAddress(), OptionCampaignExcludeIpsFromTracking::ACTION_CLICK);
    }
