...
class List_subscribersController extends Controller
{
    ...
    public function actionIndex($list_uid)
    {
        ...
        foreach ($subscribers as $subscriber) {
            ...
            foreach ($fields as $field) {
                if ($field->tag == 'EMAIL') {
                    $record[$field->tag] = html_encode($subscriber->getDisplayEmail());
                    continue;
                }
                ...
                $record[$field->tag] = html_encode($value);
            }
    ...
    public function actionView($list_uid, $subscriber_uid)
    {
        ...
        foreach ($fields as $field) {
            if ($field->tag == 'EMAIL') {
                $data['record'][$field->tag] = html_encode($subscriber->getDisplayEmail());
                continue;
            }
            ...
            $data['record'][$field->tag] = html_encode($value);
        }
    ...
    public function actionCreate($list_uid)
    {
        ...
        $subscriber->ip_address = request()->getServer('HTTP_X_MW_REMOTE_ADDR', request()->getServer('HTTP_X_REMOTE_ADDR', request()->getServer('REMOTE_ADDR')));
