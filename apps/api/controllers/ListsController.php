...
class ListsController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        foreach ($lists as $list) {
            ...
            if (!empty($list->default)) {
                $defaults = $list->default->getAttributes(['from_name', 'reply_to', 'subject']);
            }
    ...
    public function actionCreate()
    {
        ...
        // since 1.9.11
        if (($forceOptIn = $customer->getGroupOption('lists.force_optin_process', '')) && in_array($forceOptIn, array_keys($listModel->getOptInArray()))) {
            $listModel->opt_in = $forceOptIn;
        }
        if (($forceOptOut = $customer->getGroupOption('lists.force_optout_process', '')) && in_array($forceOptOut, array_keys($listModel->getOptOutArray()))) {
