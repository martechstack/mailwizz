...
class SettingsController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $this->setData([
            'pageMetaTitle'   => $this->getData('pageMetaTitle') . ' | ' . t('settings', 'Settings'),
            'pageHeading'     => t('settings', 'Settings'),
            'pageBreadcrumbs' => [
    ...
    public function actionSystem_urls()
    {
        ...
        if (request()->getIsPostRequest()) {
            foreach ($apps as $appName) {
                $optionUrl->saveAttributes([$appName . '_absolute_url' => '']);
            }
        ...
        $this->render('system-urls', compact('apps', 'scheme'));
    }
    ...
    public function actionEmail_templates($type = 'common')
    {
        ...
        if (request()->getIsPostRequest()) {
            ...
            if (isset(app_param('POST')[$model->getModelName()][$type['id']])) {
                $model->{$type['id']} = app_param('POST')[$model->getModelName()][$type['id']];
            }
                        ...
                        foreach ($regexes as $regex) {
                            if (preg_match($regex, $email)) {
                                notify()->addError(t('settings', '{email} has been matched by {regex}!', [
                                    ...
                                    '{regex}'   => sprintf('<strong>%s</strong>', html_encode($regex)),
                                ]));
                                break;
                            }
                        }
    ...
    public function actionCustomer_common()
    {
        ...
        if (request()->getIsPostRequest()) {
            ...
            if (isset(app_param('POST')[$model->getModelName()]['notification_message'])) {
                $model->notification_message = ioFilter()->purify(app_param('POST')[$model->getModelName()]['notification_message']);
            }
    ...
    public function actionCustomer_registration()
    {
        ...
        if (request()->getIsPostRequest()) {
            ...
            if (isset(app_param('POST')[$model->getModelName()]['welcome_email_content'])) {
                $model->welcome_email_content = ioFilter()->purify(app_param('POST')[$model->getModelName()]['welcome_email_content']);
            }
    ...
    public function actionCustomer_sending()
    {
        ...
        if (request()->getIsPostRequest()) {
            ...
            if (isset(app_param('POST')[$model->getModelName()]['quota_notify_email_content'])) {
                $model->quota_notify_email_content = ioFilter()->purify(app_param('POST')[$model->getModelName()]['quota_notify_email_content']);
            }
    ...
    public function actionCustomer_campaigns()
    {
        ...
        if (request()->getIsPostRequest()) {
            ...
            if (isset(app_param('POST')[$model->getModelName()]['email_header'])) {
                $model->email_header = ioFilter()->purify(app_param('POST')[$model->getModelName()]['email_header']);
            }

            if (isset(app_param('POST')[$model->getModelName()]['email_footer'])) {
                $model->email_footer = ioFilter()->purify(app_param('POST')[$model->getModelName()]['email_footer']);
            }
