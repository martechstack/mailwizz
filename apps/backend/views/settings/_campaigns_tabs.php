...
<ul class="nav nav-tabs" style="border-bottom: 0px;">
    <li class="<?php echo $controller->getAction()->getId() == 'campaign_attachments' ? 'active' : 'inactive'; ?>">
        <a href="<?php echo createUrl('settings/campaign_attachments'); ?>">
            <?php echo t('settings', 'Attachments'); ?>
        </a>
    </li>
    <li class="<?php echo $controller->getAction()->getId() == 'campaign_template_tags' ? 'active' : 'inactive'; ?>">
        <a href="<?php echo createUrl('settings/campaign_template_tags'); ?>">
            <?php echo t('settings', 'Template tags'); ?>
        </a>
    </li>
    <li class="<?php echo $controller->getAction()->getId() == 'campaign_exclude_ips_from_tracking' ? 'active' : 'inactive'; ?>">
        <a href="<?php echo createUrl('settings/campaign_exclude_ips_from_tracking'); ?>">
            <?php echo t('settings', 'Exclude IPs from tracking'); ?>
        </a>
    </li>
    <li class="<?php echo $controller->getAction()->getId() == 'campaign_blacklist_words' ? 'active' : 'inactive'; ?>">
        <a href="<?php echo createUrl('settings/campaign_blacklist_words'); ?>">
            <?php echo t('settings', 'Blacklist words'); ?>
        </a>
    </li>
    <li class="<?php echo $controller->getAction()->getId() == 'campaign_template_engine' ? 'active' : 'inactive'; ?>">
        <a href="<?php echo createUrl('settings/campaign_template_engine'); ?>">
            <?php echo t('settings', 'Template engine'); ?>
        </a>
    </li>
    <li class="<?php echo $controller->getAction()->getId() == 'campaign_webhooks' ? 'active' : 'inactive'; ?>">
        <a href="<?php echo createUrl('settings/campaign_webhooks'); ?>">
			<?php echo t('settings', 'Webhooks'); ?>
        </a>
    </li>
    <li class="<?php echo $controller->getAction()->getId() == 'campaign_misc' ? 'active' : 'inactive'; ?>">
