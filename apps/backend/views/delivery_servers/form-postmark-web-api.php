    ...
    if (!empty($collection) && $collection->itemAt('renderForm')) {
        ...
        foreach ($formFields as $fieldName => $fieldProps) {
            $index++; ?>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <?php $controller->renderPartial('_customer', compact('form')); ?>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-12">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <?php echo t('servers', 'The url where this server expects to receive webhooks requests to process bounces and complaints is: {url}. Please check and make sure the webhook has been created!', ['{url}' => sprintf('<strong>%s</strong>', $server->getDswhUrl())]); ?>
                            <br /><br />
                            <?php echo t('servers', 'You can configure your PostMark account webhook as seen here: {url}', [
                                '{url}' => CHtml::link('https://postmarkapp.com/support/article/1067-how-do-i-enable-delivery-webhooks', 'https://postmarkapp.com/support/article/1067-how-do-i-enable-delivery-webhooks', ['target' => '_blank']),
                            ]); ?>
                            <br /><br />
                            <?php echo t('servers', 'Make sure you enable Bounce and Spam Complaint events, including the above webhook url.'); ?>
                        </div>
                    </div>
                </div>
            </div>
