...
if (!empty($viewCollection) && $viewCollection->itemAt('renderContent')) {
     ...
     * @since 1.3.9.2
     */
    $itemsCount = PricePlan::model()->count(); ?>
        ...
        <div class="box-header">
            ...
            <div class="pull-right">
                <?php BoxHeaderContent::make(BoxHeaderContent::RIGHT)
                    ->addIf($controller->widget('common.components.web.widgets.GridViewToggleColumns', ['model' => $pricePlan, 'columns' => ['name', 'price', 'group_id', 'status', 'date_added']], true), $itemsCount)
                    ->add(HtmlHelper::accessLink(IconHelper::make('create') . t('app', 'Create new'), ['price_plans/create'], ['class' => 'btn btn-primary btn-flat', 'title' => t('app', 'Create new')]))
                    ->add(HtmlHelper::accessLink(IconHelper::make('refresh') . t('app', 'Refresh'), ['price_plans/index'], ['class' => 'btn btn-primary btn-flat', 'title' => t('app', 'Refresh')]))
                    ->render(); ?>
            </div>
    ...
    if (!empty($collection) && $collection->itemAt('renderGrid')) {
        $controller->widget('zii.widgets.grid.CGridView', hooks()->applyFilters('grid_view_properties', [
                    ...
                    'id'                => $pricePlan->getModelName() . '-grid',
                    ...
                    'columns' => hooks()->applyFilters('grid_view_columns', [
                        ...
                        [
                            'name'  => 'group_id',
                            'value' => '!empty($data->group_id) ? HtmlHelper::accessLink($data->customerGroup->name, array("customer_groups/update", "id" => $data->group_id), array("fallbackText" => true)) : "-"',
                            'type'  => 'raw',
                        ...
                        [
                            ...
                            'buttons'   => [
                                'update' => [
                                    ...
                                    'url'       => 'createUrl("price_plans/update", array("id" => $data->plan_id))',
                                    'imageUrl'  => null,
                                ...
                                'copy' => [
                                    ...
                                    'url'       => 'createUrl("price_plans/copy", array("id" => $data->plan_id))',
                                    'imageUrl'  => null,
                                    'options'   => ['title' => t('app', 'Copy'), 'class' => 'btn btn-primary btn-flat'],
                                    'visible'   => 'AccessHelper::hasRouteAccess("price_plans/copy")',
                                ],
                                'delete' => [
                                    'label'     => IconHelper::make('delete'),
                                    'url'       => 'createUrl("price_plans/delete", array("id" => $data->plan_id))',
                                    'imageUrl'  => null,
