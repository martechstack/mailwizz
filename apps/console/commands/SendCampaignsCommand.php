...
class SendCampaignsCommand extends ConsoleCommand
{
    ...
    public function init()
    {
        ...
        $this->_useTempQueueTables = app_param('send.campaigns.command.useTempQueueTables', false);
    ...
    public function getFeedbackIdHeaderValue(Campaign $campaign, ListSubscriber $subscriber, Lists $list, Customer $customer)
    {
        ...
        $searchReplace = hooks()->applyFilters('feedback_id_header_format_tags_search_replace', $searchReplace);
    ...
    public function stdout($message, bool $timer = true, string $separator = "\n", bool $store = false)
    {
        ...
        $message = hooks()->applyFilters('console_command_send_campaigns_stdout_message', $message, $this);
    ...
    protected function processSubscribersLoop(array $params = [])
    {
        ...
        foreach ($subscribers as $index => $subscriber) {
            ...
            if ($blCheckInfo = $subscriber->getIsBlacklisted(['checkZone' => EmailBlacklist::CHECK_ZONE_CAMPAIGN])) {
                if ($blCheckInfo->customerBlacklist) {
                    $this->logDelivery($subscriber, $blCheckInfo->reason, CampaignDeliveryLog::STATUS_BLACKLISTED, '', $server, $campaign);
                } else {
            ...
            $emailParams['headers'] = [
                ['name' => $headerPrefix . 'Campaign-Uid',   'value' => $campaign->campaign_uid],
                ['name' => $headerPrefix . 'Subscriber-Uid', 'value' => $subscriber->subscriber_uid],
                ['name' => $headerPrefix . 'Customer-Uid',   'value' => $customer->customer_uid],
                ['name' => $headerPrefix . 'Customer-Gid',   'value' => (string)intval($customer->group_id)], // because of sendgrid
                ['name' => $headerPrefix . 'Delivery-Sid',   'value' => (string)intval($server->server_id)], // because of sendgrid
                ['name' => $headerPrefix . 'Tracking-Did',   'value' => (string)intval($server->tracking_domain_id)], // because of sendgrid
            ...
            hooks()->doAction('console_command_send_campaigns_before_send_to_subscriber', $campaign, $subscriber, $customer, $server, $emailParams);

            // since 1.3.5.9
            $emailParams = hooks()->applyFilters('console_command_send_campaigns_before_send_to_subscriber', $emailParams, $campaign, $subscriber, $customer, $server);
            ...
            try {
                ...
                if (!empty($campaign->send_group_id)) {
                    ...
                    if (!mutex()->acquire($sendGroupHashKey, 10)) {
                        throw new Exception('Unable to acquire the send group mutex');
                    }

                    if (cache()->get($sendGroupHashKey)) {
                        $mustSend = false;
                        $sent     = true;
                        $response = 'Sent by another campaign in same sending group!';
                    } else {
            ...
            if (!$sent) {
                if ($status == CampaignDeliveryLog::STATUS_GIVEUP && $campaign->customer->getGroupOption('quota_counters.campaign_giveup_emails', 'no') == 'yes') {
                    $server->logUsage(['force_customer_countable' => true]);
                }
            }
        ...
        if (!empty($domainPolicySubscribers)) {
            ...
            if ($params['domainPolicySubscribersCounter'] <= $params['domainPolicySubscribersMaxRounds']) {
                ...
                $this->stdout(sprintf(
                    ...
                    is_countable($domainPolicySubscribers) ? count($domainPolicySubscribers) : 0
                ));
                $this->stdout('', false);
    ...
    protected function logDelivery(ListSubscriber $subscriber, string $message, string $status, string $messageId, DeliveryServer $server, Campaign $campaign): bool
    {
        ...
        if (!empty($server) && !empty($server->server_id)) {
    ...
    protected function prepareEmail(ListSubscriber $subscriber, DeliveryServer $server, Campaign $campaign)
    {
        ...
        // since 1.9.27
        $emailSubject = !empty($emailSubject) ? (string)$emailSubject : (string)$campaign->subject;
        ...
        // since 1.3.9.3
        $emailParams = hooks()->applyFilters('console_command_send_campaigns_after_prepare_email', $emailParams, $campaign, $subscriber, $server);
    ...
    protected function notifyCustomerReachingQuota(array $params = [])
    {
        ...
        $emailTemplate  = $optionEmailTemplate->common;
    ...
    protected function handleCampaignTimewarp(Campaign $campaign, int $count)
    {
        ...
        if ((time() - (int)strtotime((string)$campaign->started_at)) > 36 * 3600) {
            return 0;
        }
