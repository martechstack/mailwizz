...
class ListSubscriber extends ActiveRecord
{
    ...
    public function getIsBlacklisted(array $params = [])
    {
        ...
        // check since 1.3.4.7
        if ((string)$this->status === self::STATUS_BLACKLISTED) {
    ...
    {
        return (string)$this->status === self::STATUS_UNAPPROVED;
    }
    ...
    {
        return (string)$this->status === self::STATUS_UNCONFIRMED;
    }
    ...
    {
        return (string)$this->status === self::STATUS_UNSUBSCRIBED;
    }
    ...
    {
        return (string)$this->status === self::STATUS_DISABLED;
    }
    ...
    {
        return (string)$this->status === self::STATUS_MOVED;
    }
    ...
    public function loadAllCustomFieldsWithValues(): array
    {
        ...
        foreach (ListField::getAllByListId((int)$this->list_id) as $field) {
            ...
            $fields['[' . $field['tag'] . ']'] = html_encode(implode(', ', $value));
        }
    ...
    public function handleApprove(bool $forcefully = false): self
    {
        ...
        $updateProfileUrl = container()->get(OptionUrl::class)->getFrontendUrl('lists/' . $this->list->list_uid . '/update-profile/' . $this->subscriber_uid);
        $unsubscribeUrl   = container()->get(OptionUrl::class)->getFrontendUrl('lists/' . $this->list->list_uid . '/unsubscribe/' . $this->subscriber_uid);
        $searchReplace    = [
    ...
    public function sendCreatedNotifications(): self
    {
        ...
        $params  = CommonEmailTemplate::getAsParamsArrayBySlug(
            ...
            CMap::mergeArray($fieldsTags, [
                ...
                '[DETAILS_URL]'    => container()->get(OptionUrl::class)->getCustomerUrl(sprintf('lists/%s/subscribers/%s/update', $this->list->list_uid, $this->subscriber_uid)),
    ...
    public function createOptinHistory(): self
    {
        ...
        try {
            ...
            $optinHistory->optin_ip         = request()->getUserHostAddress();
    ...
    public function confirmOptinHistory(): self
    {
        ...
        try {
            ...
            $optinHistory->confirm_ip         = request()->getUserHostAddress();
    ...
    public function createOptoutHistory(): self
    {
        ...
        try {
            ...
            $optoutHistory->optout_ip         = request()->getUserHostAddress();
    ...
    public function confirmOptoutHistory(): self
    {
        ...
        try {
            ...
            $optoutHistory->confirm_ip         = request()->getUserHostAddress();
    ...
    public function unsubscribeByCampaign(Campaign $campaign): bool
    {
        try {
            ...
            if (empty($count)) {
                $ipAddress = !is_cli() ? request()->getUserHostAddress() : '127.0.0.1';
