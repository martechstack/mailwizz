...
class AllCustomersListsSubscribersFilters extends ListSubscriber
{
    ...
    {
        collect(iterator_to_array($this->getSubscribers()))
            ->map(function (ListSubscriber $subscriber) {
                return $subscriber->subscriber_id;
            })
            ->chunk(500)
            ->each(function (Illuminate\Support\Collection $subscriberIds) {
                $this->confirmSubscribersByIds($subscriberIds->all());
            });
    }
            ...
            if (!empty($subscribers)) {
                ...
                foreach ($emails as $emailsChunk) {
                    ...
                    db()->createCommand()->delete('{{customer_email_blacklist}}', ['and',
                        ...
                        ['in', 'customer_id', [$this->customer->customer_id]],
                    ]);
    ...
    public function unsubscribeSubscribers(): void
    {
        collect(iterator_to_array($this->getSubscribers()))
            ...
            ->chunk(500)
            ->each(function (Illuminate\Support\Collection $subscriberIds) {
                $this->unsubscribeSubscribersByIds($subscriberIds->all());
            });
    }
    ...
    public function disableSubscribers(): void
    {
        collect(iterator_to_array($this->getSubscribers()))
            ...
            ->chunk(500)
            ->each(function (Illuminate\Support\Collection $subscriberIds) {
                $this->disableSubscribersByIds($subscriberIds->all());
            });
    }
    ...
    public function blacklistSubscribers(): void
    {
        collect(iterator_to_array($this->getSubscribers()))
            ...
            ->chunk(500)
            ->each(function (Illuminate\Support\Collection $subscriberIds) {
                $this->blacklistSubscribersById($subscriberIds->all());
            });
    }
    ...
    public function deleteSubscribers(): int
    {
        ...
        collect(iterator_to_array($this->getSubscribers()))
            ...
            ->chunk(500)
            ->each(function (Illuminate\Support\Collection $subscriberIds) use (&$count) {
                $count += $this->deleteSubscribersByIds($subscriberIds->all());
            });
