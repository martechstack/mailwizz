...
class DeliveryServer extends ActiveRecord
{
    ...
    public function getBounceServersArray(): array
    {
        ...
        foreach ($models as $model) {
            $_options[$model->server_id] = sprintf('%s - %s(%s)', strtoupper((string)$model->service), $model->hostname, $model->username);
        }
    ...
    public function getParamsArray(array $params = []): array
    {
        ...
        if (!empty($customer)) {
            ...
            $hlines = $optionCustomerServers->getCustomHeaders();
        }
     ...
     * @return $this
     */
    public function logUsage(array $params = []): self
    {
        // since 1.3.5.5
        if (
        ...
        if (
            $this->getCanHaveHourlyQuota() || $this->getCanHaveDailyQuota() || $this->getCanHaveMonthlyQuota() ||
            (!empty($log->customer_id) && $log->customer_countable == DeliveryServerUsageLog::TEXT_YES)
        ) {
            $log->save(false);
    ...
    {
        return (string)$this->status === self::STATUS_IN_USE;
    }
    ...
    {
        $url = container()->get(OptionUrl::class)->getFrontendUrl(sprintf('dswh/%d', $this->server_id));
        if (is_cli()) {
            return $url;
        }
    ...
    public function getFormFieldsDefinition(array $fields = []): array
    {
        ...
        $fields = hooks()->applyFilters('delivery_server_form_fields_definition', $fields, $this);
    ...
    public function getProviderUrl(): string
    {
        ...
        $url = hooks()->applyFilters('delivery_server_get_provider_url', $url, $this);
