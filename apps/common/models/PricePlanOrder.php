...
class PricePlanOrder extends ActiveRecord
{
    ...
    public function getHtmlPaymentFrom(string $headingTag = 'strong', string $separator = '<br />'): string
    {
        ...
        // 1.5.0
        $html = hooks()->applyFilters('price_plan_order_get_html_payment_from', $html, $customer);
    ...
    public function getHtmlPaymentTo(string $headingTag = 'strong', string $separator = '<br />'): string
    {
        ...
        // 1.5.0
        $html = hooks()->applyFilters('price_plan_order_get_html_payment_to', $html, $customer);
    ...
    {
        return (string)$this->status === self::STATUS_COMPLETE;
    }
    ...
    {
        return (string)$this->status === self::STATUS_DUE;
    }
    ...
    protected function afterSave()
    {
        if (
            ...
            (string)$this->status === self::STATUS_COMPLETE
        ) {
