...
class Customer extends ActiveRecord
{
    ...
    public function getIsRemovable(): bool
    {
        ...
        if (in_array($this->status, [self::STATUS_PENDING_DELETE])) {
            return false;
        }
    ...
    public function _validateMinimumAge(string $attribute, array $params = []): void
    {
        ...
        $minimum = container()->get(OptionCustomerRegistration::class)->getMinimumAge();
    ...
    public function logTakeQuotaAction(array $data = []): bool
    {
        ...
        if ($logger === null) {
            ...
            $filePath = Yii::getPathOfAlias('common.runtime') . '/' . $fileName;
        ...
        $data = CMap::mergeArray($data, [
            ...
            'is_cli'        => is_cli(),
            'is_ajax'       => is_ajax(),
            'ip_address'    => is_cli() ? '' : request()->getUserHostAddress(),
    ...
    protected function beforeDelete()
    {
        ...
        // since 1.3.5
        if ((string)$this->status !== self::STATUS_PENDING_DELETE) {
