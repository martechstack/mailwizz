 ...
 * @method SendingDomain verified()
 * @method SendingDomain signingEnabled()
 */
class SendingDomain extends ActiveRecord
{
    ...
    public function scopes()
    {
        $scopes = new CMap([
            'verified' => [
                ...
                'params'    => [':vf' => self::TEXT_YES],
            ],
            'signingEnabled' => [
                'condition' => $this->getTableAlias(false, false) . '.`signing_enabled` = :se',
                'params'    => [':se' => self::TEXT_YES],
            ],
        ]);
        return CMap::mergeArray($scopes->toArray(), parent::scopes());
    }
    ...
    public function getDnsTxtDkimSelectorToAdd(): string
    {
        ...
        // since 1.3.5.9
        $record = hooks()->applyFilters('sending_domain_get_dns_txt_dkim_record', $record, $this);
    ...
    public function getDnsTxtSpfRecordToAdd(): string
    {
        ...
        if (!($spf = DnsTxtHelper::getDefaultSpfValue())) {
            ...
            $criteria->addInCondition('t.type', ['smtp', 'smtp-amazon']);
        ...
        // since 1.3.5.9
        $record = hooks()->applyFilters('sending_domain_get_dns_txt_spf_record', $record, $this, $smtpHosts);
     ...
     */
    public function findVerifiedByEmailForCustomer(string $email, int $customer_id): ?self
    {
     ...
     * @return SendingDomain|null
     */
    public function findVerifiedByEmailForSystem(string $email): ?self
    {
        if (!FilterVarHelper::email($email)) {
            return null;
        }
     ...
     * @return SendingDomain|null
     */
    public function findVerifiedByEmail(string $email, ?int $customer_id = null): ?self
    {
        $domain = null;

        if ((int)$customer_id > 0) {
            $domain = $this->findVerifiedByEmailForCustomer($email, (int)$customer_id);
        }
