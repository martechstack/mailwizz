...
class Campaign extends ActiveRecord
{
    ...
    public function search()
    {
        ...
        return new CActiveDataProvider(get_class($this), hooks()->applyFilters('campaign_model_search_data_provider_config', [
            'criteria'      => $criteria,
            'pagination'    => [
    ...
    {
        return (string)$this->status === self::STATUS_PROCESSING;
    }
    ...
    {
        return (string)$this->status === self::STATUS_SENDING;
    }
    ...
    {
        return (string)$this->status === self::STATUS_PENDING_APPROVE;
    }
    ...
    {
        return (string)$this->status === self::STATUS_PENDING_SENDING;
    }
    ...
    {
        return (string)$this->status === self::STATUS_PENDING_DELETE;
    }
    ...
    public function saveStatus(string $status = ''): bool
    {
        ...
        try {
            ...
            if ((string)$this->status === self::STATUS_PROCESSING && !$this->getStartedAt()) {
    ...
    public function tryReschedule(): bool
    {
        ...
        try {
            ...
            $cron              = CronExpression::factory($this->option->cronjob);
    ...
    public function getTotalDeliveryTime(): string
    {
        if (
            ...
            ($startedAt = (int)strtotime($this->started_at)) === ($finishedAt = (int)strtotime($this->finished_at))
        ) {
            return $this->t('N/A');
        }
    ...
    public function getBulkActionsList(): array
    {
        ...
        // since 1.9.17
        $actions = hooks()->applyFilters('campaign_model_get_bulk_actions_list', $actions, $this);
    ...
    public function sendStatsEmail(): bool
    {
        ...
        $params = CommonEmailTemplate::getAsParamsArrayBySlug(
            ...
            [
                ...
                '[CAMPAIGN_OVERVIEW_URL]' => container()->get(OptionUrl::class)->getBackendUrl('campaigns/' . $this->campaign_uid . '/overview'),
    ...
    protected function beforeSave()
    {
        ...
        } elseif ((string)$this->status === self::STATUS_SENT) {
            $this->finished_at = MW_DATETIME_NOW;
        } elseif ((string)$this->status === self::STATUS_DRAFT) {
    ...
    protected function createCountSubscribersCriteria(?CDbCriteria $mergeCriteria = null): ?CDbCriteria
    {
        ...
        // since 1.6.4
        return hooks()->applyFilters('campaign_model_count_subscribers_criteria', $criteria, $this);
    }
