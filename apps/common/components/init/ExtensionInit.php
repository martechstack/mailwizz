...
abstract class ExtensionInit extends CApplicationComponent
{
    ...
    final public function getDirName(): string
    {
        ...
        return $_dirName = basename((string)dirname((string)$reflection->getFilename()));
    }
    ...
    {
        return str_replace('-', '_', $this->getPathAlias());
    }
     ...
     * @return string
     */
    final public function createAbsoluteUrl(string $route, array $params = [], string $ampersand = '&'): string
    {
        return createAbsoluteUrl($this->getRoute($route), $params, $ampersand);
    }
    ...
    final public function setData($key, $value = null)
    {
        if (!is_array($key) && $value !== null) {
            $this->getData()->mergeWith([$key => $value], false);
        } elseif (is_array($key)) {
