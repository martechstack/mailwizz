...
class CampaignHelper
{
    ...
    public static function parseContent(string $content, Campaign $campaign, ListSubscriber $subscriber, bool $appendBeacon = false, ?DeliveryServer $server = null): array
    {
        ...
        if ($appendBeacon && !empty($subscriber->subscriber_id)) {
            $beaconUrl   = container()->get(OptionUrl::class)->getFrontendUrl('campaigns/' . $campaign->campaign_uid . '/track-opening/' . $subscriber->subscriber_uid);
            $beaconImage = CHtml::image($beaconUrl, '', ['width' => 1, 'height' => 1]);
    ...
    public static function transformLinksForTracking(string $content, Campaign $campaign, ListSubscriber $subscriber, bool $canSave = false, bool $isPlainText = false): string
    {
        ...
        $content      = $collection->itemAt('content');
        $trackingUrls = $collection->itemAt('trackingUrls');

        if (!isset($trackingUrls[$cacheKey])) {
            $trackingUrls[$cacheKey] = [];
            $baseUrl                 = container()->get(OptionUrl::class)->getFrontendUrl();
            ...
            $urls = (array)array_combine($urls, $matches[0]);
            $foundUrls = [];
    ...
    public static function embedContentImages(string $content, Campaign $campaign): array
    {
        ...
        $extensions  = app_param('files.images.extensions', []);
    ...
    public static function htmlToText(string $content): string
    {
        ...
        if ($html2text === null) {
            ...
            if (!is_cli()) {
                $html2text->setBaseUrl(container()->get(OptionUrl::class)->getCurrentAppUrl());
            }
        }
    ...
    public static function extractTemplateUrls(string $content): array
    {
        ...
        $urls[$hash] = array_unique(array_map(['CHtml', 'decode'], array_map('trim', $matches[5])));
    ...
    public static function getCommonTagsSearchReplace(string $content, Campaign $campaign, ?ListSubscriber $subscriber = null, ?DeliveryServer $server = null): array
    {
        ...
        $subject = (string)str_replace(array_keys($searchReplace), array_values($searchReplace), $campaign->subject);
    ...
    {
        return hooks()->applyFilters('common_helper_parsed_field_value_by_list_field_value_tag_info', [
    ...
    {
        $fieldValue = $collection->itemAt('fieldValue');
