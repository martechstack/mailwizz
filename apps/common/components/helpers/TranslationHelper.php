...
class TranslationHelper
{
    ...
    public static function importFromPhpFiles(string $languagePath, bool $canAddNewLanguage = false): array
    {
        ...
        if ((!file_exists($languagePath) || !is_dir($languagePath)) && !is_readable($languagePath)) {
            $result['error'] = t('translations', 'Please make sure the folder "{name}" is readable!', ['{name}' => $languagePath]);
            return $result;
        }
        ...
        try {
            ...
            $result['error'] = t('translations', 'The language "{name}" is not valid', ['{name}' => $languageAndRegionCode]);
            return $result;
        }
        ...
        if (empty($languageModel)) {
            if (!$canAddNewLanguage) {
                $result['error'] = t('translations', 'No corresponding language found in the database for {name}', ['{name}' => $languageAndRegionCode]);
                return $result;
            }
            ...
            if (!$languageModel->save()) {
                $result['error'] = t('translations', 'The language "{name}" could not be created', ['{name}' => $languageAndRegionCode]);
                return $result;
            }
        }
        ...
        foreach ($categories as $category) {
            ...
            if (!is_file($categoryFile) || pathinfo($categoryFile, PATHINFO_EXTENSION) !== 'php' || !is_readable($categoryFile)) {
                $error[] = t('translations', 'Please make sure the file "{name}" is readable and in the correct format!', ['{name}' => $categoryFile]);
                continue;
            }
            ...
            if (!is_array($data)) {
                $error[] = t('translations', 'Wrong data format for category - "{name}"', ['{name}' => $categoryFile]);
                continue;
            }

            foreach ($data as $message => $translation) {
                if (empty($message)) {
                    $error[] = t('translations', 'Empty message found for category - "{name}" - skipping', ['{name}' => $categoryName]);
                    continue;
                }
                ...
                if (empty($sourceMessage)) {
                    ...
                    if (!$sourceMessage->save()) {
                        $error[] = t('translations', 'Error saving the source message for category - "{name}" - skipping', ['{name}' => $categoryName]);
                        continue;
                    }
                }
        ...
        if ($affected == 0) {
            $error[] = t('translations', 'No new translations added');
        }
        $result['success'] = true;
        $result['error']   = !empty($error) ? implode(PHP_EOL, $error) : '';
        return $result;
    }
}
